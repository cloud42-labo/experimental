...
8. 最後に再取得して、GitHubのmerge状態とNotionのTask/後続Taskが一致していることを確認する。**TaskがDoneへ遷移した場合に限り**、利用中のAI Handoffが `workflow_state = completed` であることも確認する。Taskが未完了なら、外部イベント待ち等で再開可能なhandoffは `active` のまま保持する。
