# 未来の店舗生存サバイバルゲーム — Design Doc（実装ベース・改訂版 v3、v0.10.0時点）
...
>
> **v0.10.0での変更**: e-Statキーの保持方法を、メモリ保持のみ（リロードで再入力必須）
> から、このブラウザのlocalStorageへの保存＋次回自動入力に変更した（4.12節）。研修で
> 共有・貸与PCを使う参加者向けに、保存済みキーを明示的に削除するボタンも追加した。
> 本書はこの最新（v0.10.0）の実装を記述する。
...
その場で取得する。キーは実行中はJS変数として保持し、**このブラウザのlocalStorageにのみ**
保存して次回以降の自動入力に使う（v0.10.0〜）。配布ファイルへの埋め込みやサーバーへの
送信は行わない。研修等の共有・貸与PC向けに、起動画面から保存済みキーを明示的に削除できる。
取得したデータ自体（人口・経済センサス）はページを閉じれば消える（キャッシュしない）。
...
* **店舗プロファイル（任意）**: 年間売上高・年間粗利額・年間販管費・正社員数・
  ...
  使用）
* **e-Stat APIキー入力欄**（`type=password`、表示/隠すトグル付き。入力するとこのブラウザの
  `localStorage`に保存し次回自動入力する。「保存したキーを削除」ボタンで即時消去できる。
  v0.10.0〜、4.12節参照）
...
> この行き止まりについては、4.13節にある「戦略的店舗」トグル（Dのdisqualifyを限定的に
...
* GDD v6.1が「一旦削除」とした円建てP&Lコマンドの復活ではない —
  実際の売上・原価率・人件費率を計算する仕組みは今も存在しない（4.13節参照）。
...
### 4.12 e-Statキーのブラウザ保存（v0.10.0）

これまでキーはJS変数（`ESTAT_APP_ID`）にのみ保持し、リロードのたびに再入力が必要だった。
研修本番での利用を想定すると、参加者が毎回同じ端末を使うとは限らない一方、個人の
自端末で複数回試す場合の再入力は単純に手間なので、`localStorage`（キー名
`storeSurvivalSim.eStatAppId`）に保存し、次回以降`#appIdInput`へ自動入力する方式に変更した。

* `appIdInput`への`input`イベントで都度保存（空にすると`localStorage`からも削除）
* 起動画面に「保存したキーを削除」ボタンを追加。共有・貸与PCでの利用後に手動で消せる
* `localStorage`が使えない環境（プライベートブラウジング等の一部ブラウザ）でも例外で
  機能全体が止まらないよう、読み書きはすべて`try/catch`で包み、失敗時は従来どおり
  再入力を求めるだけに留める（キー入力自体はブロックしない）
* 配布ファイル（`index.html`自体）にキーを埋め込む変更ではない。保存先は利用者本人の
  ブラウザのみで、e-Stat公式API以外への送信もしない

### 4.13 未実装のGDD要素・今後の検討事項
...
* 「戦略的店舗」トグル（4.13節）を実装するかどうか、するとしたらD以外の選択肢にも
  効かせるかどうかは未定。
