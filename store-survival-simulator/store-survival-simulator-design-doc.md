...
`weights`（年少/生産年齢/老年の客層適性重み）、`laborDependency`（依存する労働力層）、
...
* ゲーム開始時に半径5,000m（`MAX_RADIUS_M`）分を先読みするため、開始後に店舗位置を
  その範囲外まで大きく動かすと「周辺のメッシュが見つかりません」というエラーになる
  （再取得はできない設計。許容範囲内での位置調整のみ想定）。
