...
* 入力UIは起動画面（`#setupScreen`の店舗プロファイル欄）とゲーム内編集（4.7節）の両方に、
  既存5項目と同じ体裁で追加する。
* **未入力（0）でも破綻しない**: `人件費(t)=0`、`その他販管費=P.sga`となり、営業利益の式は
  `粗利(t) − P.sga`に縮退する。この場合、人件費率は算出不能として`—`を表示する。

代替案として「ACから人件費率を落とす」こともありうるが、それはStoryのスコープ変更にあたる
ため、採らない場合はChris（ChatGPT、`EPIC-SS-01`のPM）へ差し戻す。
...
#### 4.14.7 未確定事項（要Chris確認）

本モデルは`SS-01-S02-2`の受入条件を満たすが、以下1点はStoryの解釈に関わるため、実装
（`SS-01-S02-3`）着手前に`EPIC-SS-01`のPM（Chris）の確認を得る。

* **カネ施策がP&Lの金額を動かさないことを許容するか。** 4.14.1.1の通り、人材施策・品揃え
  施策は新規の仮定値ゼロで金額を動かせるが、カネ施策（`kane_genka`／`kane_honbu`）だけは
  人口セグメントと無関係なため`finTier`の定性ブーストにとどまる。もし「全施策が円建ての
  P&Lを動かすこと」がStoryの要件であれば、`SS-01-S02-1`で確定した「新規の仮定値を持ち
  込まない」方針の一部緩和が必要になり、`SS-01-S02`全体の再決定になる。
