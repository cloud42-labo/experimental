...
5. **Persistence-failure exception.** Whenever the terminal Notion write does not succeed, persistence-before-notification (rule 3) is waived for that one post so the workflow still has a valid stop path:
   ...
   - `COMPLETED` is never used while `persisted_to_notion: false` — rule 4's invariant holds without exception: a write failure never lets Chris declare completion, even if acceptance criteria were met by the same event;
   - if a stop condition other than completion was already determined independently of the Notion write itself (for example `FAILED_LIMIT` from rule 6/8, `LOOP_DETECTED`, `TURN_LIMIT`, or a `HUMAN_REQUEST`/`CAPABILITY_FAILURE` reached for a reason other than this write), the status used is that already-determined tag — the write failure does not relabel it, since the write failing is a persistence problem, not a change in why the workflow stopped;
   - in every other case — including when acceptance criteria were met and the only reason `COMPLETED` cannot be posted is this write failure — Chris chooses the status by failure kind: `CAPABILITY_FAILURE` for unreachable/timeout failures, or `HUMAN_REQUEST` for permission or validation failures, since those need the owner (or Notion schema/access owner) to act before a write can ever succeed; when the underlying event met acceptance criteria, the message additionally includes `completion_candidate: true` and states plainly that the task is not yet complete because its terminal state is unpersisted;
   ...
   - when that re-mention arrives, Chris backfills the terminal record in Notion synchronously before confirming closure; if the backfilled record reflects a `completion_candidate: true` event, that backfill is what first makes `COMPLETED` the confirmed status — Chris posts `COMPLETED` at that point, not before; for every other preserved tag, the backfill confirms and closes under that same tag;
...
**Exception:** rule 6's third-unresolvable-blocker case is carved out of this order rather than following it. When the event is specifically an unresolvable `blocked` closure that is the task's third unsuccessful attempt, rule 6 and rule 8 already require `FAILED_LIMIT`, even though the same event would otherwise also satisfy `HUMAN_REQUEST`/`CAPABILITY_FAILURE`; that case always resolves to `FAILED_LIMIT`, not to whichever tag ranks higher in the general order above. The general order in this section governs every other simultaneous-condition case, including a first- or second-attempt unresolvable blocker, where rule 6 already specifies `HUMAN_REQUEST`/`CAPABILITY_FAILURE` and there is no conflict to resolve.
