...
4. The workflow reaches twenty agent turns. A turn is counted each time Chris or an agent posts a non-terminal message carrying the minimum message contract — a dispatch, an evidence result, or a Chris evaluation/instruction — to the workflow thread; owner messages, Slack delivery retries or duplicate deliveries (attempt-lifecycle rule 3), non-contract natural-language chatter, and the terminal Slack message itself (whichever stop tag it carries) do not count. The count is cumulative across the whole workflow, including the parent and every parallel branch (rule 10), and is not reset by attempt increments. Chris checks the count before posting any non-terminal message: if 19 non-terminal turns have already been counted, the 20th would-be non-terminal message is never posted — in particular, if that message would be a dispatch, Chris does not mention an agent or start new work with it. Instead Chris stops immediately with `TURN_LIMIT`; that stop message is the sole 20th message, not turn 21, and is excluded from the budget like any terminal message. This pre-dispatch check is what keeps the twentieth turn from ever being a dispatch that launches an agent.
...
- **Test E4 — Turn limit:** using at least one parallel dispatch so branch turns count toward the same workflow-wide total (stop condition 4), drive the workflow to nineteen non-terminal turns such that Chris's next action would be a dispatch. Confirm Chris's pre-dispatch budget check catches this before mentioning the next agent, so that dispatch never happens; confirm Chris stops with `TURN_LIMIT` as the sole 20th message instead, that the terminal message itself is not counted as an additional turn, that the terminal status is persisted in Notion before the final Slack post, and that the workflow does not self-restart.
